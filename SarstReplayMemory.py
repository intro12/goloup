...
class SarstReplayMemory:
    ...
    def __init__(self, capacity, state_size, prioritize=True, priority_epsilon=0.001, priority_alpha=0.5):
        ...
        self.screens = np.zeros((self.memory_capacity, 84, 84), dtype = np.uint8)
    ...
    def add_to_memory(self, state, action, reward, next_state, next_state_is_terminal, loss):
        """
        ...
        if self.prioritize:
            ...
            sample_priority = (abs(loss) + self.priority_epsilon)**self.priority_alpha
    ...
    def get_state(self, index):
        state = self.screens[index - 10 + 1:index + 1, :, :]
